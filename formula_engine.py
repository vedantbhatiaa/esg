...
Converted from formula_engine.ipynb so app.py can import it.
"""
...
EF = {
    "Natural Gas": 56.1/1000, "Coal": 96.1/1000, "Propane": 63.1/1000,
    "Fuel Oil": 77.4/1000,    "Diesel": 74.1/1000, "Petrol": 69.3/1000,
    "Biomass": 0.0,            "Waste tires": 47.5/1000,
    "LPG": 56.1/1000,          "Other": 71.9/1000,
}
WASTE_TIRE_HV = 36.23
GJ_TO_MWH = 1/3.6
SCOPE2_ELEC_EF = 0.45
...
def calculate(d: TemplateInputs) -> TemplateOutputs:
    ...
    s1_ng   = d.nat_gas * EF["Natural Gas"];        s1_coal = d.coal_sub * EF["Coal"]
    s1_prop = d.propane * EF["Propane"];             s1_fo   = d.fuel_oil_heavy_a * EF["Fuel Oil"]
    s1_die  = d.diesel  * EF["Diesel"];              s1_pet  = d.petrol * EF["Petrol"]
    s1_bio  = d.biomass * EF["Biomass"];             s1_wt   = wt_gj * EF["Waste tires"]
    s1_lpg  = d.lpg     * EF["LPG"];                s1_oth  = d.other_fuels * EF["Other"]
    ...
    total_co2 = scope1 + scope2;  co2_kpi = sdiv(total_co2, d.production)
...
def validate_submission(inp, out, prev_out=None, threshold=20.0):
    ...
    if prev_out:
        for name, cur, prev in [("Total Energy", out.total_energy, prev_out.total_energy),
                                  ("Total CO2", out.total_co2, prev_out.total_co2)]:
            if prev and abs(cur-prev)/max(abs(prev),1)*100 > threshold:
                flags.append(ValidationFlag("warning", f"{name}: {(cur-prev)/abs(prev)*100:+.1f}% YoY",""))
    if not flags:
...
def get_benchmarks(out, bench_df=None):
    ...
    for col, (fq25, fmed, fq75, unit, lb) in STATIC.items():
        ...
        if bench_df is not None and not bench_df.empty and col in bench_df.columns:
            ...
            if len(vals) >= 4: q25, med, q75 = np.percentile(vals, [25,50,75])
        results.append(BenchmarkResult(col, getattr(out,col,0.0), q25, med, q75, unit, lb))
    return results

def fmt_num(val, decimals=0):
    try: return f"{float(val):,.{decimals}f}" if decimals else f"{float(val):,.0f}"
...
def build_template_dataframe(inp, out):
    import pandas as pd
    return pd.DataFrame([{"Company":inp.company,"Year":inp.year,
        "production":inp.production, "total_energy":out.total_energy,
        "energy_kpi":out.energy_kpi, "total_co2":out.total_co2,
        "co2_kpi":out.co2_kpi, "water_kpi":out.water_kpi,
