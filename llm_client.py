...
- M6: _call() now validates both the API key AND the endpoint URL before
      making a network request. If the endpoint is still the placeholder
      string "YOUR-RESOURCE" the method returns the mock response rather than
      firing a request to an invalid host and raising an unhandled exception.
"""
